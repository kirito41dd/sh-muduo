...
// 显式实例化，编译器生成模板的声明和定义实例，可以把模板定义写在cpp中。对应于隐式实例化
...
{
    // std::numeric_limits<T>::digits10 的值是类型 T 能无更改地表示的底 10 位数，
    // 即任何拥有这么多十进制有效数字的数能转换成 T 的值并转换回十进制形式，而不因舍入或上溢而更改
    static_assert(kMaxNumericSize - 10 > std::numeric_limits<double>::digits10,   // <limits>
                  "kMaxNumericSize is not large enough");
...
LogStream& LogStream::operator<<(double v)
{
  if (buffer_.avail() >= kMaxNumericSize)
  {
    int len = snprintf(buffer_.current(), kMaxNumericSize, "%.12g", v); // 自动选f格式或e格式
    buffer_.add(len);
  }
  return *this;
}
